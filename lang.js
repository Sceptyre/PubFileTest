...
function setOptionsList(profileElement) {
  console.log(profileElement);

  for (i = 0; i < profileElement.options.length; i++) {
    let e = profileElement.options[i];
    e.value = e.text;
    e.text = profileList[e.value].displayName;

    console.log(e);

  };

  document.getElementById("profiles-button").children[0].innerText = profiles.selectedOptions[0].text;
};

let observer = new MutationObserver((mutations, instance) => {
  let profileElement = document.getElementById("profiles");
  if(profileElement && profileElement.options.length > 0) {
    instance.disconnect();
    fetch(window.location.origin + "/f9_profiles_en.json").then(res => res.json()).then(data => {
      profileList = data;
      setOptionsList(profileElement);
    });
  };
